...
TEST_F(BackendTest, Basic)
{
    ...
    boost::asio::spawn(ioc, [&done, &work, &ioc](boost::asio::yield_context yield) {
        boost::log::core::get()->set_filter(clio::log_severity >= clio::Severity::WRN);
        std::string keyspace =
            "clio_test_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
        ...
        for (auto& config : configs)
        {
...

            {
                ...
                {
                    EXPECT_THROW({ backend->writeNFTs(std::move(nftData)); }, std::runtime_error);
                    EXPECT_THROW({ backend->writeNFTTransactions(std::move(parsedNFTTxs)); }, std::runtime_error);
                }

                backend->writeLedgerObject(std::string{accountIndexBlob}, lgrInfoNext.seq, std::string{accountBlob});
                backend->writeSuccessor(
                    uint256ToString(Backend::firstKey), lgrInfoNext.seq, std::string{accountIndexBlob});
                backend->writeSuccessor(
                    std::string{accountIndexBlob}, lgrInfoNext.seq, uint256ToString(Backend::lastKey));
...

            {
                ...
                {
                    EXPECT_THROW({ backend->fetchNFT(nftID, lgrInfoNext.seq, yield); }, std::runtime_error);
            ...
            }
            {
                ...
                backend->writeSuccessor(
                    uint256ToString(Backend::firstKey), lgrInfoNext.seq, uint256ToString(Backend::lastKey));
                    ...
                    }) == state[lgrInfo.seq - 1].end())
                {
                    for (size_t i = 0; i < objs.size(); ++i)
                    {
                        if (i + 1 < objs.size())
                            ...
                            backend->writeSuccessor(
                                std::string{objs[i].first}, lgrInfo.seq, uint256ToString(Backend::lastKey));
                    }
                    if (state.count(lgrInfo.seq - 1))
                        ...
                        backend->writeSuccessor(
                            uint256ToString(Backend::firstKey), lgrInfo.seq, std::string{objs[0].first});
                }
            ...
            auto checkLedger = [&](auto lgrInfo, auto txns, auto objs, auto accountTx) {
                ...
                // EXPECT_TRUE(retLgr);
                // EXPECT_EQ(ledgerInfoToBlob(*retLgr),
                // ledgerInfoToBlob(lgrInfo));
...
TEST_F(BackendTest, cache)
{
    ...
    for (size_t i = 0; i < objs.size(); ++i)
    {
        objs[i] = {ripple::uint256{i * 100 + 1}, {(unsigned char)i, (unsigned char)i * 2, (unsigned char)i + 1}};
    }
    cache.update(objs, curSeq);
    {
    ...
    for (size_t i = 0; i < objs.size(); ++i)
    {
        objs2[i] = {ripple::uint256{i * 100 + 50}, {(unsigned char)i, (unsigned char)i * 3, (unsigned char)i + 5}};
    }
    ...
    cache.update(objs, curSeq);
    {
        ...
        allObjs.clear();
        std::copy_if(
            objs.begin(), objs.end(), std::back_inserter(allObjs), [](auto obj) { return obj.blob.size() > 0; });
        std::copy(objs2.begin(), objs2.end(), std::back_inserter(allObjs));
        std::sort(allObjs.begin(), allObjs.end(), [](auto a, auto b) { return a.key < b.key; });
        std::optional<LedgerObject> succ = {{firstKey, {}}};
        size_t idx = 0;
        while ((succ = cache.getSuccessor(succ->key, curSeq)))
        {
...
TEST_F(BackendTest, cacheBackground)
{
...

    {
        ...
        for (size_t i = 0; i < 10; i++)
        {
            ...
            auto cacheObj = cache.get(obj.key, curSeq);
            ASSERT_TRUE(cacheObj);
            auto newObj = std::find_if(objs1.begin(), objs1.end(), [&](auto o) { return o.key == obj.key; });
            if (newObj == objs1.end())
            {
                ASSERT_EQ(*cacheObj, obj.blob);
...

    {
        auto objs = bObjs;
        objs.clear();
        std::copy(bObjs.begin() + 10, bObjs.begin() + 20, std::back_inserter(objs));
...

    {
        auto objs = bObjs;
        objs.clear();
        std::copy(bObjs.begin() + 20, bObjs.begin() + 30, std::back_inserter(objs));
        cache.update(objs, startSeq, true);
    }
    {
        for (auto& obj : objs1)
    ...
    cache.update(objs3, curSeq);
    {
        ...
        {
            auto cacheObj = cache.get(obj.key, curSeq);
            if (std::find_if(objs3.begin(), objs3.end(), [&](auto o) { return o.key == obj.key; }) == objs3.end())
            {
                ASSERT_TRUE(cacheObj);
                ASSERT_EQ(*cacheObj, obj.blob);
                ASSERT_FALSE(cache.get(obj.key, startSeq));
        ...
        for (size_t i = 0; i < 30; i++)
        {
            ...
            auto cacheObj = cache.get(obj.key, curSeq);
            auto newObj = std::find_if(objs1.begin(), objs1.end(), [&](auto o) { return o.key == obj.key; });
            auto delObj = std::find_if(objs3.begin(), objs3.end(), [&](auto o) { return o.key == obj.key; });
            if (delObj != objs3.end())
            {
                ASSERT_FALSE(cacheObj);
                ASSERT_FALSE(cache.get(obj.key, startSeq));
...
TEST_F(BackendTest, cacheIntegration)
{
    ...
    boost::asio::spawn(ioc, [&ioc, &done, &work](boost::asio::yield_context yield) {
        boost::log::core::get()->set_filter(clio::log_severity >= clio::Severity::WRN);
        std::string keyspace =
            "clio_test_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
        boost::json::object cassandraConfig{
        ...
        for (auto& config : configs)
        {
            ...
            auto checkLedger = [&](auto lgrInfo, auto objs) {
                ...
                EXPECT_EQ(ledgerInfoToBlob(*retLgr), ledgerInfoToBlob(lgrInfo))
                    ...
                    << "; retLgr parentHash:" << retLgr->parentHash << "; lgr Info parentHash:" << lgrInfo.parentHash;
