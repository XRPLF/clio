...
TEST_F(BackendCassandraTest, Basic)
{
    ...
    boost::asio::spawn(ctx, [this, &done, &work](boost::asio::yield_context yield) {
...

        {
            ...
            backend->writeSuccessor(uint256ToString(Backend::firstKey), lgrInfoNext.seq, std::string{accountIndexBlob});
            backend->writeSuccessor(std::string{accountIndexBlob}, lgrInfoNext.seq, uint256ToString(Backend::lastKey));
