...
TEST_F(BackendCassandraTest, Basic)
{
    ...
    boost::asio::spawn(
        ctx, [this, &done, &work](boost::asio::yield_context yield) {
            ...
            [[maybe_unused]] auto binaryStringToUint256 =
                [](auto const& bin) -> ripple::uint256 {
                ripple::uint256 uint;
                return uint.fromVoid((void const*)bin.data());
            };
            [[maybe_unused]] auto ledgerInfoToBinaryString =
                [](auto const& info) {
            ...
            EXPECT_FALSE(backend->fetchLedgerBySequence(lgrInfo.seq + 1, yield)
                             .has_value());
            ...
            }
            {
                ...
                retLgr =
                    backend->fetchLedgerBySequence(lgrInfoNext.seq - 1, yield);
                EXPECT_EQ(
                    RPC::ledgerInfoToBlob(*retLgr),
                    RPC::ledgerInfoToBlob(lgrInfoOld));
                EXPECT_NE(
                    ...
                    RPC::ledgerInfoToBlob(lgrInfoNext));
                retLgr =
                    backend->fetchLedgerBySequence(lgrInfoNext.seq - 2, yield);
                EXPECT_FALSE(
                    backend->fetchLedgerBySequence(lgrInfoNext.seq - 2, yield)
                        .has_value());
...

            {
                ...
                auto [nftTxns, cursor] =
                    backend->fetchNFTTransactions(nftID, 100, true, {}, yield);
                ...
                auto obj =
                    backend->fetchLedgerObject(key256, lgrInfoNext.seq, yield);
                EXPECT_TRUE(obj);
                EXPECT_STREQ(
            ...
            auto generateNextLedger = [seed](auto lgrInfo) {
                ...
                std::shuffle(
                    lgrInfo.txHash.begin(), lgrInfo.txHash.end(), randomEngine);
                std::shuffle(
