...
public:
    ...
    networkValidatedLedgers() const = 0;

    virtual DOSGuard&
    dosGuard() const = 0;

    virtual ReportingETL&
    reportingETL() const = 0;
...
class ApplicationImp : public Application
{
    ...
    mutable boost::asio::io_context socketContext_;
...
public:
    ...
    networkValidatedLedgers() const override
    {
        return *ledgers_;
    }
    ...
    ReportingETL&
    reportingETL() const override
    {
        return *etl_;
    }

    RPC::WorkQueue&
    workQueue() const override
    {
        return *queue_;
    }
    ...
    start() override
    {
        ...
        for (auto i = 0; i < config_->etlWorkers; ++i)
            etlWorkers.emplace_back([this]() { etlContext_.run(); });

        std::vector<std::thread> socketWorkers = {};
        for (auto i = 0; i < config_->socketWorkers - 1; ++i)
            socketWorkers.emplace_back([this]() { socketContext_.run(); });

        socketContext_.run();

        for (auto& worker : socketWorkers)
            worker.join();
