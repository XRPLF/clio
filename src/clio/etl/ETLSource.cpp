...
ETLSourceImpl<Derived>::ETLSourceImpl(
    ...
    : resolver_(boost::asio::make_strand(app.etl()))
...
PlainETLSource::PlainETLSource(
    ...
    , ws_(std::make_unique<
          ...
          boost::asio::make_strand(app.etl())))
{
}
                ...
                [this, startAgain](auto ec) {
                    ...
                    if (startAgain)
                    {
                        ws_ = std::make_unique<boost::beast::websocket::stream<
                            ...
                            boost::asio::make_strand(app_.etl()), *sslCtx_);
    ...
    boost::asio::yield_context& yield) const
{
    ...
    try
    {
        ...
        tcp::resolver resolver{app_.rpc()};

        BOOST_LOG_TRIVIAL(trace) << "Creating websocket";
        auto ws =
            std::make_unique<websocket::stream<beast::tcp_stream>>(app_.rpc());
