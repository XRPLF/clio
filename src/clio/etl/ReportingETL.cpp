...
#include <clio/subscriptions/SubscriptionManager.h>
#include <cstdlib>
#include <iostream>
#include <string>
#include <thread>
...
ReportingETL::loadInitialLedger(uint32_t startingSequence)
{
    ...
    if (!stopping_)
    {
        ...
        app_.backend().writeNFTTransactions(std::move(insertTxResult.nfTokenTxData));
    }
...
ReportingETL::buildNextLedger(org::xrpl::rpc::v1::GetLedgerResponse& rawData)
{
    ...
    app_.backend().writeAccountTransactions(std::move(insertTxResult.accountTxData));
    app_.backend().writeNFTs(std::move(insertTxResult.nfTokensData));
    app_.backend().writeNFTTransactions(std::move(insertTxResult.nfTokenTxData));
    BOOST_LOG_TRIVIAL(debug) << __func__ << " : "
                             << "wrote account_tx";
...
ReportingETL::runETLPipeline(uint32_t startSequence, int numExtractors)
{
    ...
    for (size_t i = 0; i < numExtractors; ++i)
    {
        ...
        extractors.emplace_back([this,
            ...
            while (
                (!finishSequence_ || currentSequence <= *finishSequence_) &&
                app_.ledgers().waitUntilValidatedByNetwork(currentSequence) &&
    ...
    std::thread transformer{[this,
        ...
        while (!writeConflict)
        {
            ...
            {
                deleting_ = true;
                app_.etl().post([this, &minSequence]() {
...
ReportingETL::monitor()
{
    ...
    while (true)
    {
        ...
        else if (app_.ledgers().waitUntilValidatedByNetwork(nextSequence, 1000))
        {
            BOOST_LOG_TRIVIAL(info)
                << __func__ << " : "
...
ReportingETL::monitorReadOnly()
{
    ...
    if (!rng)
        if (auto net = app_.ledgers().getMostRecent())
            latestSequence = *net;
        else
            return;
    else
    ...
    while (true)
    {
            ...
            rng && rng->maxSequence >= latestSequence)
        {
            ...
            latestSequence = latestSequence + 1;
              ...
              // publish them
            app_.ledgers().waitUntilValidatedByNetwork(latestSequence, 1000);
    }
}
...
ReportingETL::ReportingETL(Application const& app)
    : app_(app), publishStrand_(app.etl())
{
