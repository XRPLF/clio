...
{
    add(dosGuard, json_["dos_guard"], &parseDosGuardConfig);
    add(etlSources, json_["etl_sources"], &parseETLSources);
    add(cache, json_["cache"], &parseCache);
    add(server, json_["server"], &parseServerConfig);
    add(sslKeyFile, json_["ssl_key_file"], &parseString<cfgOPTIONAL>);
    add(sslCertFile, json_["ssl_cert_file"], &parseString<cfgOPTIONAL>);
    add(subscriptionWorkers,
        json_["subscription_workers"],
        &parseUInt32<cfgREQUIRED>,
        1);
    ...
    add(readOnly, json_["read_only"], &parseBool<cfgREQUIRED>);
