...
#include <clio/subscriptions/Message.h>
#include <memory>
...
public:
    static std::unique_ptr<SubscriptionManager>
    make_SubscriptionManager(Application const& app)
    {
        return std::make_unique<SubscriptionManager>(app);
    }

    SubscriptionManager(Application const& app);
