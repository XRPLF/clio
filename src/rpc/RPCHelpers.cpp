...
{
    xrpl::uint256 cursorIndex = beast::zero;
...
{
    if (auto const amount = meta->getDeliveredAmount())
        return amount;
...
toJson(xrpl::STBase const& obj)
{
    boost::json::value value =
        boost::json::parse(obj.getJson(xrpl::JsonOptions::none).toStyledString());
    ...
    if (canHaveDeliveredAmount(txn, meta)) {
        if (auto amt = getDeliveredAmount(txn, meta, meta->getLgrSeq(), date)) {
            metaJson["delivered_amount"] =
                toBoostJson(amt->getJson(xrpl::JsonOptions::include_date));
        } else {
...
{
    boost::json::value value =
        boost::json::parse(meta.getJson(xrpl::JsonOptions::none).toStyledString());
...
{
    boost::json::value value =
        boost::json::parse(sle.getJson(xrpl::JsonOptions::none).toStyledString());
...
toJson(xrpl::LedgerHeader const& lgrInfo, bool const binary, std::uint32_t const apiVersion)
{
    ...
    if (binary) {
        ...
        header[JS(close_time_iso)] = xrpl::to_string_iso(lgrInfo.closeTime);
...
{
    auto const firstNFTPage = xrpl::keylet::nftpage_min(accountID);
    auto const lastNFTPage = xrpl::keylet::nftpage_max(accountID);

    // check if nextPage is valid
    if (nextPage != beast::zero and firstNFTPage.key != (nextPage & ~xrpl::nft::pageMask))
        return std::unexpected{Status{RippledError::rpcINVALID_PARAMS, "Invalid marker."}};

    // no marker, start from the last page
    xrpl::uint256 const currentPage = nextPage == beast::zero ? lastNFTPage.key : nextPage;
...
)
{
    ...
    auto const isNftMarkerNonZero =
        startHint == std::numeric_limits<uint32_t>::max() and hexCursor != beast::zero;
...
)
{
    ...
    if (hexMarker.isNonZero()) {
        ...
        if (!hintDir)
            return std::unexpected{Status(xrpl::rpcINVALID_PARAMS, "Invalid marker.")};
        ...
        for (;;) {
            ...
            if (!ownerDir) {
                return std::unexpected{
                    Status(xrpl::rpcINVALID_PARAMS, "Owner directory not found.")
                };
            }
        ...
        try {
            ...
            if (saOwnerFundsLimit >= saTakerGets) {
                ...
                offerJson["taker_gets_funded"] =
                    toBoostJson(saTakerGetsFunded.getJson(xrpl::JsonOptions::none));
                offerJson["taker_pays_funded"] = toBoostJson(
                    std::min(
                        saTakerPays, xrpl::multiply(saTakerGetsFunded, dirRate, saTakerPays.issue())
                    )
                        .getJson(xrpl::JsonOptions::none)
                );
            }

            xrpl::STAmount const saOwnerPays = (xrpl::parityRate == offerRate)
                ? saTakerGetsFunded
...
)
{
    ...
    return xrpl::Book{{pays, payIssuer}, {gets, getIssuer}, domainID};
}
...
parseBook(boost::json::object const& request)
{
    ...
    if (!xrpl::to_currency(getCurrency, boost::json::value_to<std::string>(takerGets["currency"])))
        return std::unexpected{Status{RippledError::rpcDST_AMT_MALFORMED}};
    ...
    if (takerPays.contains("issuer")) {
        ...
        if (!xrpl::to_issuer(payIssuer, boost::json::value_to<std::string>(takerPays.at("issuer"))))
    ...
    if (takerGets.contains("issuer")) {
        ...
        if (!xrpl::to_issuer(
            ...
            return std::unexpected{Status{
                RippledError::rpcDST_ISR_MALFORMED, "Invalid field 'taker_gets.issuer', bad issuer."
            }};
        }
    ...
    return xrpl::Book{{payCurrency, payIssuer}, {getCurrency, getIssuer}, domainID};
}
