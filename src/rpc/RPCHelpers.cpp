...
accountFromStringStrict(std::string const& account)
{
    ...
    if (blob && xrpl::publicKeyType(xrpl::makeSlice(*blob))) {
        ...
        publicKey = util::parseBase58Wrapper<xrpl::PublicKey>(xrpl::TokenType::AccountPublic, account);
    }
...
deserializeTxPlusMeta(data::TransactionAndMetadata const& blobs, std::uint32_t seq)
{
    ...
    std::shared_ptr<xrpl::TxMeta> const m = std::make_shared<xrpl::TxMeta>(tx->getTransactionID(), seq, *meta);
...
{
    boost::json::value value = boost::json::parse(obj.getJson(xrpl::JsonOptions::Values::None).toStyledString());
...
)
{
    if (canHaveDeliveredAmount(txn, meta)) {
        if (auto amt = getDeliveredAmount(txn, meta, meta->getLgrSeq(), date)) {
            metaJson["delivered_amount"] = toBoostJson(amt->getJson(xrpl::JsonOptions::Values::IncludeDate));
        } else {
 ...
 */
static bool
canHaveMPTIssuanceID(std::shared_ptr<xrpl::STTx const> const& txn, std::shared_ptr<xrpl::TxMeta const> const& meta)
{
...
{
    boost::json::value value = boost::json::parse(meta.getJson(xrpl::JsonOptions::Values::None).toStyledString());
...
{
    boost::json::value value = boost::json::parse(sle.getJson(xrpl::JsonOptions::Values::None).toStyledString());
...
)
{
    ...
    auto const isNftMarkerNonZero = startHint == std::numeric_limits<uint32_t>::max() and hexCursor != beast::kZero;
    auto const isNftMarkerZero = startHint == std::numeric_limits<uint32_t>::max() and hexCursor == beast::kZero;
    ...
    return traverseOwnedNodes(
        backend, xrpl::keylet::ownerDir(accountID), hexCursor, startHint, sequence, limit, yield, atOwnedNode
    );
}
...
)
{
    ...
    if (hexMarker.isNonZero()) {
        ...
        for (;;) {
            ...
            if (!ownerDir)
                return std::unexpected{Status(xrpl::RpcInvalidParams, "Owner directory not found.")};
...
)
{
    if (auto const blob = backend->fetchLedgerObject(keylet.key, lgrInfo.seq, context.yield); blob) {
        return std::make_shared<xrpl::SLE const>(xrpl::SerialIter{blob->data(), blob->size()}, keylet.key);
    }
...
parseRippleLibSeed(boost::json::value const& value)
{
    ...
    auto const result = xrpl::decodeBase58Token(boost::json::value_to<std::string>(value), xrpl::TokenType::None);
...
)
{
    ...
    auto const allowBalance = [&]() {
        ...
        if (amendmentCenter.isEnabled(yield, data::Amendments::fixFrozenLPTokenTransfer, sequence)) {
            auto const issuerBlob = backend.fetchLedgerObject(xrpl::keylet::account(issuer).key, sequence, yield);

            if (!issuerBlob)
                return false;

            xrpl::SLE const issuerSle{
                xrpl::SerialIter{issuerBlob->data(), issuerBlob->size()}, xrpl::keylet::account(issuer).key
            };
            ...
            if (issuerSle.isFieldPresent(xrpl::sfAMMID)) {
                ...
                xrpl::SLE const ammSle{xrpl::SerialIter{ammBlob->data(), ammBlob->size()}, ammKeylet.key};
...
)
{
    if (isXRP(pays) && !isXRP(payIssuer)) {
        return std::unexpected{Status{
            RippledError::RpcSrcIsrMalformed, "Unneeded field 'taker_pays.issuer' for XRP currency specification."
        }};
    }

    if (!isXRP(pays) && isXRP(payIssuer)) {
        return std::unexpected{
            Status{RippledError::RpcSrcIsrMalformed, "Invalid field 'taker_pays.issuer', expected non-XRP issuer."}
        };
    }

    if (xrpl::isXRP(gets) && !xrpl::isXRP(getIssuer)) {
        return std::unexpected{Status{
            RippledError::RpcDstIsrMalformed, "Unneeded field 'taker_gets.issuer' for XRP currency specification."
        }};
    }

    if (!xrpl::isXRP(gets) && xrpl::isXRP(getIssuer)) {
        return std::unexpected{
            Status{RippledError::RpcDstIsrMalformed, "Invalid field 'taker_gets.issuer', expected non-XRP issuer."}
        };
    }
...
parseBook(boost::json::object const& request)
{
    if (!request.contains("taker_pays"))
        return std::unexpected{Status{RippledError::RpcInvalidParams, "Missing field 'taker_pays'"}};

    if (!request.contains("taker_gets"))
        return std::unexpected{Status{RippledError::RpcInvalidParams, "Missing field 'taker_gets'"}};

    if (!request.at("taker_pays").is_object())
        return std::unexpected{Status{RippledError::RpcInvalidParams, "Field 'taker_pays' is not an object"}};

    if (!request.at("taker_gets").is_object())
        return std::unexpected{Status{RippledError::RpcInvalidParams, "Field 'taker_gets' is not an object"}};
    ...
    if (!xrpl::toCurrency(payCurrency, boost::json::value_to<std::string>(takerPays.at("currency"))))
        return std::unexpected{Status{RippledError::RpcSrcCurMalformed}};
    ...
    if (takerPays.contains("issuer")) {
        if (!takerPays.at("issuer").is_string())
            return std::unexpected{Status{RippledError::RpcInvalidParams, "takerPaysIssuerNotString"}};
    ...
    if (takerGets.contains("issuer")) {
        if (!takerGets["issuer"].is_string())
            return std::unexpected{Status{RippledError::RpcInvalidParams, "taker_gets.issuer should be string"}};

        if (!xrpl::toIssuer(getIssuer, boost::json::value_to<std::string>(takerGets.at("issuer")))) {
            return std::unexpected{
                Status{RippledError::RpcDstIsrMalformed, "Invalid field 'taker_gets.issuer', bad issuer."}
            };
        }

        if (getIssuer == xrpl::noAccount()) {
            return std::unexpected{
                Status{RippledError::RpcDstIsrMalformed, "Invalid field 'taker_gets.issuer', bad issuer account one."}
            };
        }
    } else {
