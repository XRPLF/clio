...
doAccountTx(Context const& context)
{
    ...
    if (request.contains("marker"))
    {
        ...
        if (obj.contains("seq"))
        {
            ...
            transactionIndex = boost::json::value_to<std::uint32_t>(obj.at("seq"));
        }
        ...
        if (obj.contains("ledger"))
        {
            ...
            ledgerIndex = boost::json::value_to<std::uint32_t>(obj.at("ledger"));
        }
    ...
    if (request.contains("ledger_index"))
    {
        ...
        auto ledgerIndex = boost::json::value_to<uint32_t>(request.at("ledger_index"));
    ...
    if (request.contains("ledger_hash"))
    {
        ...
        auto lgrInfo = context.backend->fetchLedgerByHash(
            ledgerHash, context.yield);
