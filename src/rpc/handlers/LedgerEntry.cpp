...
LedgerEntryHandler::process(LedgerEntryHandler::Input const& input, Context const& ctx) const
{
    ...
    if (input.index) {
        ...
        key =
            xrpl::keylet::account(*util::parseBase58Wrapper<xrpl::AccountID>(*(input.accountRoot)))
                .key;
    } else if (input.did) {
        ...
        auto const currency = xrpl::to_currency(
        ...
        key = xrpl::keylet::escrow(*id, util::integralValueAs<uint32_t>(input.escrow->at(JS(seq))))
                  .key;
        ...
        auto const getIssuerFromJson = [](auto const& assetJson) {
            ...
            auto const currency =
                xrpl::to_currency(boost::json::value_to<std::string>(assetJson.at(JS(currency))));
        ...
        if (input.bridgeAccount) {
            ...
            key =
                xrpl::keylet::xChainClaimID(input.bridge->value(), input.chainClaimId.value()).key;
        } else {
...
tag_invoke(boost::json::value_to_tag<LedgerEntryHandler::Input>, boost::json::value const& jv)
{
    ...
    static auto const kINDEX_FIELD_TYPE_MAP =
        std::unordered_map<std::string, xrpl::LedgerEntryType>{
            {JS(index), xrpl::ltANY},
    ...
    auto const parseBridgeFromJson = [](boost::json::value const& bridgeJson) {
        auto const lockingDoor =
            *util::parseBase58Wrapper<xrpl::AccountID>(boost::json::value_to<std::string>(
                bridgeJson.at(xrpl::sfLockingChainDoor.getJsonName().c_str())
            ));
        auto const issuingDoor =
            *util::parseBase58Wrapper<xrpl::AccountID>(boost::json::value_to<std::string>(
                bridgeJson.at(xrpl::sfIssuingChainDoor.getJsonName().c_str())
            ));
        auto const lockingIssue =
            parseIssue(bridgeJson.at(xrpl::sfLockingChainIssue.getJsonName().c_str()).as_object());
        auto const issuingIssue =
            parseIssue(bridgeJson.at(xrpl::sfIssuingChainIssue.getJsonName().c_str()).as_object());
