...
VaultInfoHandler::process(VaultInfoHandler::Input const& input, Context const& ctx) const
{
    ...
    xrpl::STLedgerEntry const vaultSle{
        xrpl::SerialIter{vaultLedgerObject->data(), vaultLedgerObject->size()},
    ...
    Output response;
    response.vault = toBoostJson(vaultSle.getJson(xrpl::JsonOptions::none));
    response.vault.as_object()[JS(shares)] =
        toBoostJson(issuanceSle.getJson(xrpl::JsonOptions::none));
