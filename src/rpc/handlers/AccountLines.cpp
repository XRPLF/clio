...
)
{
    ...
    bool const lineAuthPeer = (flags & (not viewLowest ? xrpl::lsfLowAuth : xrpl::lsfHighAuth)) != 0u;
    bool const lineNoRipple = (flags & (viewLowest ? xrpl::lsfLowNoRipple : xrpl::lsfHighNoRipple)) != 0u;
    bool const lineNoRipplePeer = (flags & (not viewLowest ? xrpl::lsfLowNoRipple : xrpl::lsfHighNoRipple)) != 0u;
...
AccountLinesHandler::process(AccountLinesHandler::Input const& input, Context const& ctx) const
{
    ...
    auto const peerAccountID = input.peer ? accountFromStringStrict(*(input.peer)) : std::optional<xrpl::AccountID>{};
