...
PostgresBackend::PostgresBackend(boost::asio::io_context& ioc, boost::json::object const& config)
    : BackendInterface(ioc, config)
...
PostgresBackend::fetchLatestLedgerSequence(boost::asio::yield_context& yield) const
{
    PgQuery pgQuery(pgPool_);
    pgQuery("SET statement_timeout TO 10000", yield);
    auto res = pgQuery(
        "SELECT ledger_seq FROM ledgers ORDER BY ledger_seq DESC LIMIT 1", yield);
    ...
    while (true)
    {
        try
        {
            auto [objects, curCursor, warning] =
                fetchLedgerPage(cursor, minLedger, 256, 0, yield);
            if (warning)
            {
