...
private:
    static constexpr auto kCONFIG_DESCRIPTION = std::array{
        ...
        KV{.key = "forwarding.cache_timeout",
           .value =
               "Specifies the timeout duration (in seconds) for the forwarding cache used in `rippled` communication."},
        ...
        KV{.key = "cache.num_diffs",
           .value = "The number of cursors generated is the number of changed objects in the latest `cache.num_diffs` "
                    "number of ledgers. Deleted objects does not count. For more information, please read "
                    "[README.md](../src/etl/README.md) "
                    "under etl."},
        KV{.key = "cache.num_markers", .value = "The number of corountines to load the cache concurrently."},
        KV{.key = "cache.num_cursors_from_diff",
           .value = "`cache.num_cursors_from_diff` number of cursors are generated by looking at the number of changed "
                    ...
                    "it will keep reading previous ledgers until it hit `cache.num_cursors_from_diff`."},
