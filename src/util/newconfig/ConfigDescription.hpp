...
private:
    static constexpr auto configDescription = std::array{
        KV{"database.type", "Type of database to use. Default is Scylladb."},
        KV{"database.cassandra.contact_points",
        ...
        KV{"database.cassandra.secure_connect_bundle",
           "Configuration file that contains the necessary security credentials and connection details for securely "
        ...
        KV{"database.cassandra.connect_timeout",
           "The maximum amount of time in seconds the system will wait for a connection to be successfully established "
           "with the database."},
        KV{"database.cassandra.request_timeout",
           "The maximum amount of time in seconds the system will wait for a request to be fetched from database."},
        KV{"database.cassandra.username", "The username used for authenticating with the database."},
        KV{"database.cassandra.password", "The password used for authenticating with the database."},
        KV{"database.cassandra.certfile",
           "The path to the SSL/TLS certificate file used to establish a secure connection between the client and the "
           "Cassandra database."},
        KV{"allow_no_etl", "If True, no ETL nodes will run with Clio."},
        ...
        KV{"rpc.cache_timeout", "Timeout duration for the rpc request."},
        KV{"num_markers", "The number of markers is the number of coroutines to load the cache concurrently."},
        ...
        KV{"graceful_period", "Number of milliseconds server will wait to shutdown gracefully."},
        ...
        KV{"api_version.default", "Default API version Clio will run on."},
