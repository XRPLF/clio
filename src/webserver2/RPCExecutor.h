...
public:
    ...
    {
        perfLog.debug() << connection->tag() << "Adding to work queue";
        ...
        if (!rpcEngine_->post(
                [request = std::move(req), connection, this](boost::asio::yield_context yc) {
     ...
     * Try to the session shared ptr in subscription manager
...
private:
    ...
    {
        perfLog.debug() << connection->tag() << "Received request from work queue: " << request;
        ...
        try
        {
            ...
            if (!context)
            {
                perfLog.warn() << connection->tag() << "Could not create RPC context";
                log.warn() << connection->tag() << "Could not create RPC context";
                return connection->send(
            ...
            if (auto const status = std::get_if<RPC::Status>(&v))
            {
                ...
                perfLog.debug() << context->tag() << "Encountered error: " << boost::json::serialize(response);
                log.debug() << context->tag() << "Encountered error: " << boost::json::serialize(response);
            }
            else
            {
        ...
        {
            perfLog.error() << connection->tag() << "Caught exception : " << e.what();
            log.error() << connection->tag() << "Caught exception : " << e.what();
            return connection->send(
