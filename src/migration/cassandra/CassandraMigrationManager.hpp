...
/**
 * @brief The migration manager implementation for Cassandra. It will run the migration and rollback for the Cassandra
 * database.
 *
 * @tparam SupportedMigrators The migrators resgister that contains all the migrators
 */
template <typename SupportedMigrators>
class CassandraMigrationManagerBase : public MigrationManagerInterface {
...
public:
     ...
     */
    explicit CassandraMigrationManagerBase(
     ...
     */
    void
    runRollback(std::string const& name) override
    {
        migrators_.runRollback(name);
    }
...
using CassandraSupportedMigrators = MigratorsRegister<CassandraMigrationBackend>;

/// The Cassandra migration manager
using CassandraMigrationManager = CassandraMigrationManagerBase<CassandraSupportedMigrators>;
}  // namespace migration::cassandra
