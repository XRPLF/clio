...
TEST_F(MPTHelpersTest, ReferencesMptIssuance_MatchesFromAffectedNode)
{
    std::vector<xrpl::STObject> nodes;
    nodes.push_back(util::createMPTokenNode(xrpl::sfCreatedNode, defaultIssuanceID(), kAccount));
    auto const txMeta = createTxMeta(std::move(nodes));

    EXPECT_TRUE(etl::referencesMptIssuance(txMeta, createTx(xrpl::ttPAYMENT), defaultIssuanceID()));
}

TEST_F(MPTHelpersTest, ReferencesMptIssuance_NoMatchWhenIssuanceDiffers)
{
    ...
    EXPECT_FALSE(
        etl::referencesMptIssuance(txMeta, createTx(xrpl::ttPAYMENT), defaultIssuanceID())
    );
}

TEST_F(MPTHelpersTest, ReferencesMptIssuance_FailedTxIgnoresAffectedNodes)
{
    std::vector<xrpl::STObject> nodes;
    nodes.push_back(util::createMPTokenNode(xrpl::sfCreatedNode, defaultIssuanceID(), kAccount));
    auto const txMeta = createTxMeta(std::move(nodes), xrpl::tecINCOMPLETE);
...
TEST_F(MPTHelpersTest, ReferencesMptIssuance_FailedTxStillMatchesViaOwnFields)
{
    ...
    EXPECT_TRUE(etl::referencesMptIssuance(txMeta, sttx, defaultIssuanceID()));
}

TEST_F(MPTHelpersTest, ReferencesMptIssuance_NoReferenceReturnsFalse)
{
