...
TEST_F(RPCAccountMPTokensHandlerTest, DefaultParameters)
{
    ...
    xrpl::STObject const ownerDir = createOwnerDirLedgerObject(
        {xrpl::uint256{kTOKEN_INDEX1}, xrpl::uint256{kTOKEN_INDEX2}}, kTOKEN_INDEX1
    );
...
TEST_F(RPCAccountMPTokensHandlerTest, UseLimit)
{
    ...
    auto const bbs = [&]() {
        ...
        for (int i = 0; i < 50; ++i) {
            v.push_back(
                createMpTokenObject(kACCOUNT, xrpl::uint192(kISSUANCE_ID_HEX), i, 0, std::nullopt)
                    .getSerializer()
                    .peekData()
            );
        }
...
TEST_F(RPCAccountMPTokensHandlerTest, MarkerOutput)
{
    ...
    runSpawn([this, kLIMIT, kNEXT_PAGE](auto yield) {
        ...
        EXPECT_EQ(
            ...
            fmt::format("{},{}", kTOKEN_INDEX1, kNEXT_PAGE)
        );
    });
}
