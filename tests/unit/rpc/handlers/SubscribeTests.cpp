...
TEST_F(RPCSubscribeHandlerTest, StreamsLedger)
{
    ...
    runSpawn([&, this](auto yield) {
        ...
        auto const output = handler.process(input, Context{yield, session_});
        ASSERT_TRUE(output);
        EXPECT_EQ(output.result->as_object(), boost::json::parse(kEXPECTED_OUTPUT));
    });
}
...
TEST_F(RPCSubscribeHandlerTest, BooksBothSnapshotSet)
{
    ...
    ON_CALL(*backend_, doFetchSuccessorKey(xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}, kMAX_SEQ, _))
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}, kMAX_SEQ, _))
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::uint256{kPAYS20_XRP_GETS10_USD_BOOK_DIR}, kMAX_SEQ, _))
        .WillByDefault(Return(createOwnerDirLedgerObject(indexes2, kINDEX2).getSerializer().peekData()));

    // offer owner account root
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::keylet::account(getAccountIdWithString(kACCOUNT2)).key, kMAX_SEQ, _))
        .WillByDefault(Return(createAccountRootObject(kACCOUNT2, 0, 2, 200, 2, kINDEX1, 2).getSerializer().peekData()));

    // issuer account root
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::keylet::account(getAccountIdWithString(kACCOUNT)).key, kMAX_SEQ, _))
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::keylet::fees().key, kMAX_SEQ, _)).WillByDefault(Return(feeBlob));
