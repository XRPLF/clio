...
TEST_F(RPCMPTHoldersHandlerTest, NonExistLedgerViaLedgerHash2)
{
    ...
    ON_CALL(*backend_, fetchLedgerByHash(xrpl::uint256{kLEDGER_HASH}, _)).WillByDefault(Return(ledgerinfo));
    EXPECT_CALL(*backend_, fetchLedgerByHash).Times(1);
    auto const input = boost::json::parse(
        fmt::format(
            R"JSON({{
...
TEST_F(RPCMPTHoldersHandlerTest, SpecificLedgerIndex)
{
    ...
    EXPECT_CALL(
        *backend_,
        fetchMPTHolders(
            xrpl::uint192(kMPT_ID), testing::_, testing::Eq(std::nullopt), Const(specificLedger), testing::_
        )
    )
        .Times(1);
...
TEST_F(RPCMPTHoldersHandlerTest, MarkerParameter)
{
    ...
    EXPECT_CALL(
        *backend_, fetchMPTHolders(xrpl::uint192(kMPT_ID), testing::_, testing::Eq(marker), Const(30), testing::_)
    )
        .Times(1);
