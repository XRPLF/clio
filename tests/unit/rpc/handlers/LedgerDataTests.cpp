...
TEST_F(RPCLedgerDataHandlerTest, NoMarker)
{
    ...
    ON_CALL(*backend_, doFetchSuccessorKey(_, kRANGE_MAX, _)).WillByDefault(Return(xrpl::uint256{kINDEX2}));
    ...
    runSpawn([&, this](auto yield) {
        ...
        EXPECT_EQ(output.result->as_object().at("ledger"), boost::json::parse(kLEDGER_EXPECTED));
...
TEST_F(RPCLedgerDataHandlerTest, TypeFilterAMM)
{
    ...
    auto const amm = createAmmObject(kACCOUNT, "XRP", xrpl::toBase58(xrpl::xrpAccount()), "JPY", kACCOUNT2);
...
TEST_F(RPCLedgerDataHandlerTest, TypeFilterMPToken)
{
    ...
    auto const mptoken = createMpTokenObject(kACCOUNT, xrpl::makeMptID(2, getAccountIdWithString(kACCOUNT)));
