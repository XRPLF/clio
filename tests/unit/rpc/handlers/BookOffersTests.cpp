        ...
        BookOffersNormalTestBundle{
            ...
            .mockedLedgerObjects =
                std::map<xrpl::uint256, xrpl::Blob>{
                    // book dir object
                    {xrpl::uint256{kPAYS20_XRP_GETS10_USD_BOOK_DIR},
                     createOwnerDirLedgerObject({xrpl::uint256{kINDEX2}, xrpl::uint256{kINDEX2}}, kINDEX1)
                         .getSerializer()
                         .peekData()},
                    // gets issuer account object
                    {xrpl::keylet::account(account).key,
...
TEST_F(RPCBookOffersHandlerTest, LedgerNonExistViaIntSequence)
{
    ...
    ON_CALL(*backend_, fetchLedgerBySequence(30, _)).WillByDefault(Return(std::optional<xrpl::LedgerHeader>{}));
...
TEST_F(RPCBookOffersHandlerTest, Limit)
{
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::keylet::account(getAccountIdWithString(kACCOUNT2)).key, seq, _))
...
TEST_F(RPCBookOffersHandlerTest, LimitMoreThanMax)
{
    ...
    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(5);
    auto const indexes = std::vector<xrpl::uint256>(BookOffersHandler::kLIMIT_MAX + 1, xrpl::uint256{kINDEX2});
