...
constexpr auto kPAYS20_USD_GETS10_XRP_BOOK_DIR =
    "43B83ADC452B85FCBADA6CAEAC5181C255A213630D58FFD455071AFD498D0000";

constexpr auto kPAYS20_XRP_GETS10_USD_BOOK_DIR =
...
generateNormalPathBookOffersTestBundles()
{
    ...
    auto const getsXRPPaysUSDBook = getBookBase(
        rpc::parseBook(
            xrpl::to_currency("USD"), account, xrpl::xrpCurrency(), xrpl::xrpAccount(), std::nullopt
        )
            .value()
    );
    auto const getsXRPPaysUSDBookWithDomain = getBookBase(
        rpc::parseBook(
            xrpl::to_currency("USD"), account, xrpl::xrpCurrency(), xrpl::xrpAccount(), kDOMAIN
        )
            .value()
    );
    auto const getsUSDPaysXRPBook = getBookBase(
        rpc::parseBook(
            xrpl::xrpCurrency(), xrpl::xrpAccount(), xrpl::to_currency("USD"), account, std::nullopt
        )
            .value()
    );
    ...
    return std::vector<BookOffersNormalTestBundle>{
        BookOffersNormalTestBundle{
            ...
            .mockedSuccessors =
                std::map<xrpl::uint256, std::optional<xrpl::uint256>>{
                    {getsXRPPaysUSDBook, xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}},
                    {xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}, std::optional<xrpl::uint256>{}}
                },
            .mockedLedgerObjects =
                std::map<xrpl::uint256, xrpl::Blob>{
                    // book dir object
                    {xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR},
        ...
        BookOffersNormalTestBundle{
            ...
            .mockedSuccessors =
                std::map<xrpl::uint256, std::optional<xrpl::uint256>>{
                    {getsXRPPaysUSDBookWithDomain, xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}},
                    {xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}, std::optional<xrpl::uint256>{}}
                },
            .mockedLedgerObjects =
                std::map<xrpl::uint256, xrpl::Blob>{
        ...
        BookOffersNormalTestBundle{
            ...
            .mockedSuccessors =
                std::map<xrpl::uint256, std::optional<xrpl::uint256>>{
                    {getsUSDPaysXRPBook, xrpl::uint256{kPAYS20_XRP_GETS10_USD_BOOK_DIR}},
                    {xrpl::uint256{kPAYS20_XRP_GETS10_USD_BOOK_DIR}, std::optional<xrpl::uint256>{}}
                },
            .mockedLedgerObjects =
                std::map<xrpl::uint256, xrpl::Blob>{
                    // book dir object
                    {xrpl::uint256{kPAYS20_XRP_GETS10_USD_BOOK_DIR},
                     createOwnerDirLedgerObject({xrpl::uint256{kINDEX2}}, kINDEX1)
                         .getSerializer()
                         .peekData()},
...
TEST_F(RPCBookOffersHandlerTest, Limit)
{
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::uint256{kPAYS20_USD_GETS10_XRP_BOOK_DIR}, seq, _))
        .WillByDefault(
    ...
    ON_CALL(*backend_, doFetchLedgerObject(xrpl::keylet::account(issuer).key, seq, _))
        .WillByDefault(
            Return(createAccountRootObject(kACCOUNT, 0, 2, 200, 2, kINDEX1, 2, kTRANSFER_RATE_X2)
                       .getSerializer()
                       .peekData())
        );
...
TEST_F(RPCBookOffersHandlerTest, LimitMoreThanMax)
{
    ...
    EXPECT_CALL(*backend_, doFetchLedgerObject).Times(5);
    auto const indexes =
        std::vector<xrpl::uint256>(BookOffersHandler::kLIMIT_MAX + 1, xrpl::uint256{kINDEX2});
    ...
    std::vector<Blob> const bbs(
        BookOffersHandler::kLIMIT_MAX + 1, gets10XRPPays20USDOffer.getSerializer().peekData()
    );
    ON_CALL(*backend_, doFetchLedgerObjects).WillByDefault(Return(bbs));
    EXPECT_CALL(*backend_, doFetchLedgerObjects).Times(1);
