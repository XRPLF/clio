...
)
{
    ...
    xrpl::Issue const issue1(
        ...
        util::parseBase58Wrapper<xrpl::AccountID>(std::string(issuer)).value()
    );
    if (reverse) {
...
getIssue(std::string_view currency, std::string_view issuerId)
{
    ...
    if (currency.size() == 3) {
        return xrpl::Issue(
            xrpl::to_currency(std::string(currency)),
...
createLptCurrency(std::string_view assetCurrency, std::string_view asset2Currency)
{
    return xrpl::ammLPTCurrency(
        xrpl::to_currency(std::string(assetCurrency)),
        xrpl::to_currency(std::string(asset2Currency))
    );
}
...
)
{
    ...
    for (std::size_t i = 0; i < issuer.size(); ++i) {
        ...
        credential.setFieldVL(
            xrpl::sfCredentialType, xrpl::strUnHex(std::string(credType[i])).value()
        );
