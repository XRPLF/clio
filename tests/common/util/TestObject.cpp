...
{
    return util::parseBase58Wrapper<xrpl::AccountID>(std::string(id)).value();
}
...
)
{
    ...
    obj.setAccountID(xrpl::sfAccount, account.value());
    ...
    obj.setAccountID(xrpl::sfDestination, account2.value());
...
)
{
    ...
    xrpl::Issue const issue1(
        xrpl::Currency{currency}, util::parseBase58Wrapper<xrpl::AccountID>(std::string(issuer)).value()
    );
    if (reverse) {
...
getIssue(std::string_view currency, std::string_view issuerId)
{
    ...
    if (currency.size() == 3) {
        return xrpl::Issue(
            ...
            util::parseBase58Wrapper<xrpl::AccountID>(std::string(issuerId)).value()
        );
    }
    return xrpl::Issue(
        xrpl::Currency{currency}, util::parseBase58Wrapper<xrpl::AccountID>(std::string(issuerId)).value()
    );
}
...
)
{
    ...
    line.setFieldAmount(xrpl::sfHighLimit, xrpl::STAmount(getIssue(currency, highNodeAccountId), highLimit));
    line.setFieldAmount(xrpl::sfLowLimit, xrpl::STAmount(getIssue(currency, lowNodeAccountId), lowLimit));
...
data::NFT
createNft(std::string_view tokenID, std::string_view account, xrpl::LedgerIndex seq, xrpl::Blob uri, bool isBurned)
{
...
)
{
    ...
    auto account = util::parseBase58Wrapper<xrpl::AccountID>(std::string(accountId));
    tx.setAccountID(xrpl::sfAccount, account.value());
    auto amount = xrpl::STAmount(fee, false);
...
)
{
    ...
    finalFields.setAccountID(xrpl::sfOwner, account.value());
...
)
{
    ...
    amm.setFieldIssue(xrpl::sfAsset, xrpl::STIssue{xrpl::sfAsset, getIssue(assetCurrency, assetIssuer)});
    amm.setFieldIssue(xrpl::sfAsset2, xrpl::STIssue{xrpl::sfAsset2, getIssue(asset2Currency, asset2Issuer)});
    xrpl::Issue const issue1(
        xrpl::Currency{lpTokenBalanceIssueCurrency},
        util::parseBase58Wrapper<xrpl::AccountID>(std::string(accountId)).value()
    );
...
)
{
    ...
    chainOwnedClaimID.setFieldU32(xrpl::sfFlags, 0);
    chainOwnedClaimID.setAccountID(xrpl::sfOtherChainSource, getAccountIdWithString(otherChainSource));
...
)
{
    ...
    chainOwnedCreateAccountClaimID.setFieldU16(xrpl::sfLedgerEntryType, xrpl::ltXCHAIN_OWNED_CLAIM_ID);
    ...
    chainOwnedCreateAccountClaimID.setFieldU32(xrpl::sfFlags, 0);
    chainOwnedCreateAccountClaimID.setFieldArray(xrpl::sfXChainCreateAccountAttestations, xrpl::STArray{});
...
{
    // ammLPTCurrency now takes Assets; the issuer is irrelevant to the LPToken currency hash
    // (only the underlying currencies are hashed), so a fixed account is used for both.
...
)
{
    ...
    priceData.setFieldCurrency(xrpl::sfBaseAsset, xrpl::STCurrency{xrpl::sfBaseAsset, baseAssetCurrency});
    priceData.setFieldCurrency(xrpl::sfQuoteAsset, xrpl::STCurrency{xrpl::sfQuoteAsset, quoteAssetCurrency});
...
)
{
    ...
    if (expiration.has_value())
        credObj.setFieldU32(xrpl::sfExpiration, expiration.value());
...
createAuthCredentialArray(std::vector<std::string_view> issuer, std::vector<std::string_view> credType)
{
    ...
    for (std::size_t i = 0; i < issuer.size(); ++i) {
        ...
        credential.setFieldVL(xrpl::sfCredentialType, xrpl::strUnHex(std::string(credType[i])).value());
...
)
{
    ...
    vault.setFieldIssue(xrpl::sfAsset, xrpl::STIssue{xrpl::sfAsset, getIssue(assetCurrency, assetIssuer)});
    ...
    // sfLossUnrealized is SoeDefault; explicitly setting it to its default value is rejected
    // by applyTemplate in newer libxrpl, so it is left unset.
